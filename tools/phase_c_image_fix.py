...
_AUX_PATTERNS = [
    ...
    ("text_leak", re.compile(
        # (a) 従来: 看板/ロゴ/ホワイトボード/画面等 + 文字/テキスト + 除去意図。
        #     ※「画面/スクリーン」は必ず 文字/テキスト を伴う時のみ(「不要な画面を削除」=浮遊モニタ除去
        #       ＝物理系であって焼き込み文字でないため、誤ってtext_leak化しない)。
        r"(看板|ロゴ|ホワイトボード|白板|貼り紙|パッケージ|画面|スクリーン|ボード|見出し|バナー)"
        r".{0,16}(文字|テキスト|表記|見出し).{0,24}(削除|消|除去|不要|なくす)"
        # (b) 吹き出し/セリフ/発言 は焼き込みセリフ = 除去意図と近接なら単独で text_leak(良品計画8と同型)。
        r"|(吹き出し|吹出し|セリフ|発言).{0,16}(削除|消|除去|不要|なくす)"
        r"|(削除|消|除去|不要|なくす).{0,10}(吹き出し|吹出し|セリフ)"
        # (c) 「画像内(の)テキスト/文字/表記/ロゴ/マークを 変更/削除/日本語化」= 画像に焼かれた文字の是正。
        r"|画像内.{0,10}(テキスト|文字|表記|ロゴ|マーク).{0,30}(削除|消|除去|不要|なくす|変更|日本語)"
        # (d) 焼き込み明示。
