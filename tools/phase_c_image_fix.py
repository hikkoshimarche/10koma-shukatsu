...
_PRIMARY_PATTERNS = [
    ("meta_frame", re.compile(r"四角[くいで]?.{0,8}(囲|枠|空白|網掛け|ボックス)|網掛け|メタ枠|囲われた空白")),
    ("hline",      re.compile(r"横線|分断|継ぎ目|(空|建物|背景).{0,8}(分断|途切|裂)")),
    ("scale",      re.compile(r"縮尺|遠近|サイズ比|(サイズ|大きさ|比率).{0,6}(合|不自然|比|調整)|大きすぎ|小さすぎ|巨大")),
    # 白帯/クロップ: 「白い〜」の記述語で拾う(空白の"白"に誤爆しないよう 白い を要求)。
    ("white_band", re.compile(r"白い.{0,6}(空白|部分|帯|スペース)|白帯|余白|クロップ")),
]
