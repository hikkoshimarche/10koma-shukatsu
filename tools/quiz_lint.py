...
BANNED_SCHEDULE_KW = ("株主総会", "配当支払開始", "配当支払予定", "支払開始予定", "支払開始日",
                      ...
                      # v3追加: 電話・上場取引所・住所断片(現実に存在しない誤答を生みやすい単純転記)
...
CONCEPT_MAP = [
    ...
    ("従業員数", ("従業員", "社員数")),
    ("連結子会社数", ("連結子会社", "子会社数", "関連会社", "グループ会社数", "持分法適用会社")),
    ("設立", ("設立", "創業", "創立")),
    ("資本金", ("資本金",)),
...
def _fact_keys(q):
    ...
    if c:
        keys.add(("c:" + c, asof))
    # 正解の値(数値 or 名称)も鍵に。同じ答え=同じ事実の強いシグナル。
    opts = q.get("options") or []
    ci = q.get("correct")
    if isinstance(ci, int) and 0 <= ci < len(opts):
        corr = str(opts[ci]).strip()
        nums = _num_tokens(corr)
        if nums:  # 数値回答: 正規化数値集合を鍵に(単位ゆれ・言い回し無関係)
            keys.add(("v:" + ",".join(sorted(nums)), asof))
    return keys
