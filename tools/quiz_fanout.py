...
GEN_SYS = (
 ...
 "【人名trivia禁止(答えベース)】正解や選択肢が『人物の氏名(姓名)』になる設問は一切作らない。"
 "役職名(代表取締役/社長/会長/CEO/代表者/取締役/執行役員/創業者/初代/部長/担当者/顧問 等)の"
 "有無に関わらず、答えが人名なら禁止(例「代表者は誰」「創業者は誰」「採用担当者は誰」も不可)。"
 "面接で使えず学習価値が無いため。人物は事業への役割・貢献・戦略の文脈でのみ扱い、氏名を正解にしない。"
 "【Lv4は数字の暗記でなく読み方】決算数値そのものの暗記(『売上高はいくら』を複数年反復等)より、"
 "利益率・構造・戦略の理解を優先(出典に記載がある範囲のみ・Source-or-Silence厳守。無ければ無理に作らない)。"
...
def converge_locked(slug, name, corpus, target=30, max_round=5, extra="", fin_floor=8):
    ...
    for rnd in range(max_round):
        ...
        # 常時ベースで付与(以前は財務バイアス分岐が上書きして消えていた=lint落ちが減らなかった実測バグを是正)。
        ...
        # 生成バイアス: 財務が下限未満→意味ある財務を追加、上限到達→非財務のみ(anti-fabは残す=+=で追記)
        if fin_n < fin_floor:
            dyn += ("\n【重要】意味のある財務設問を多めに作れ(収益/営業利益/税引前利益/当期利益/"
                    "親会社帰属利益/総資産/ROE/配当/キャッシュフロー等を各1問ずつ・異なる指標で)。")
        elif fin_n >= fin_cap:
            dyn += ("\n【重要】財務数値は上限。以降は財務を作らず、事業セグメント/会社概要/沿革/"
                    "製品・サービス/人名・役員 のみで作ること。")
        # 【収量改善2】多様性: 既出の正解/設問を回避リストで渡し、本文の別領域から作らせる
        # (広範社でラウンド間に同じ~8概念を反復生成しseen_qで大量dropしていた実測に基づく)。
        if accepted:
            _cov = []
            for a in accepted[-26:]:
                _c = str((a.get("options") or [""])[a.get("correct", 0)] if a.get("options") else "")[:16]
                if _c:
                    _cov.append(_c)
            if _cov:
                dyn += ("\n【多様性(厳守)】次はすでに出題済み。これらと重複せず、本文の《別の》事業/製品/"
                        "セグメント/子会社/沿革/理念/技術/拠点/人物から作れ: " + " / ".join(_cov))
